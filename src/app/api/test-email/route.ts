...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `- MAILERSEND_API_KEY: ${process.env.MAILERSEND_API_KEY ? process.env.MAILERSEND_API_KEY.substring(0, 15) + "..." : "NOT SET"}`,
    );
...
export async function GET() {
  ...
  const smtpStatus = {
    ...
    mailerSendApiKey: process.env.MAILERSEND_API_KEY
      ? process.env.MAILERSEND_API_KEY.substring(0, 15) + "..."
      : "not set",
  ...
  if (!hasMailerSendConfig && !hasSmtpConfig && !hasGmailConfig) {
    if (!smtpStatus.hasMailerSendApiKey)
      issues.push("Missing MAILERSEND_API_KEY environment variable (recommended)");
  ...
  const isConfigured =
    hasMailerSendConfig || ((hasSmtpConfig || hasGmailConfig) && smtpStatus.hasContactFromEmail);
